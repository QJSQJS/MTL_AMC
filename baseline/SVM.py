...
cnn2 = keras.Sequential(
    [
        ...
        Dropout(dr),
        Dense(len(classes), kernel_initializer='he_normal', name="dense2",kernel_regularizer=regularizers.l2(0.01)),
        #layers.BatchNormalization(),
        Activation('linear'),
        #Activation('softmax'),
        Reshape([len(classes)])
...
cnn2.compile(loss='squared_hinge', optimizer='adam')
cnn2.summary()
...
save_dir = './rice_svm_1024_liner_426/'

if not os.path.exists(save_dir):
    os.mkdir(save_dir)


filepath = save_dir + 'rice_SVM_20.wts.h5'
...
duration = end - start
print('base-svm testing time = ' + str(duration) + 's')
...
print(acc_cnn2)
np.save(save_dir + 'rice_svm20.npy', acc_cnn2)
